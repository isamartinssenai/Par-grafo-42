        ...
        <form id="livroForm">
            ...
            <div class="field-group">
                    ...
                    <select id="genero"  value={{ $livro->genero}} name="genero" required>
                        ...
                        <option value="ficcao" {{ $livro->genero == 'ficcao' ? 'selected' : '' }}>Ficção Científica</option>
                        ...
                        <option value="historia" {{ $livro->genero == 'historia' ? 'selected' : '' }}>História</option>
...
<style>
  ...
  .swal-badge-edit {
    ...
    align-items: center;
    justify-content: center;
    box-shadow: 0 10px 25px rgba(58,134,179,0.35);
    font-family: 'Montserrat', sans-serif;
  }
    ...
    Swal.fire({
      ...
      html: `
        <div class="swal-card-clean">
          <div class="swal-badge-edit">✓</div>
          <div class="swal-content-clean">
            <h2>Livro atualizado</h2>
            <p>As informações foram alteradas com sucesso.</p>
            <button id="swalCloseEdit">Continuar</button>
          </div>
        </div>
      `,
      didOpen: () => {
        document.getElementById("swalCloseEdit").addEventListener("click", () => Swal.close());
      }
    });
  }

  // Exemplo: dispara o alerta ao clicar no botão com id 'meuid'
  document.getElementById("meuid")?.addEventListener("click", alertaAlterado);
</script>
</body>
</html>